...
export class StatusScreen {
    ...
    init() {
        ...
        overlay.addEventListener('click', (e) => {
            // Click handling inside the status overlay.
            if (e.target === overlay) this.close();
        });

        overlay.addEventListener('click', (e) => {
            // Click handling for tabs, rows, and actions.
            ...
            const selectItemBtn = e.target.closest('[data-action="select-item"]');
            if (selectItemBtn) {
                const itemId = selectItemBtn.dataset.item;
                if (!itemId) return;
    ...
    _buildRow({ title, right, tags = [], action = '' }) {
        const tagHtml = `<div class="status-row-tags">${tags.map(t => `<span class="status-tag">${t}</span>`).join('')}</div>`;
    ...
    _buildItemRow({ id, name, count, tags = [], isDisabled = false }) {
        const tagHtml = `<div class="status-row-tags">${tags.map(t => `<span class="status-tag">${t}</span>`).join('')}</div>`;
        return `
            <button type="button"
                class="status-row status-item ${isDisabled ? 'status-skill--disabled' : ''}"
